# Tornado Tracks Webmap Research Brief
...
This fits the existing site well: MapLibre is already installed, map routes are
already treated as full-screen when the path ends with `/map`, and the
temperature project has reusable patterns for URL-driven state, map controls,
legends, summary panels, lazy route loading, and static JSON/GeoJSON data.
