...
function isRouteGeoJson(value: unknown): value is RouteGeoJson {
    ...
    if (geoJson.type === 'Feature') return geoJson.geometry?.type === 'LineString' || geoJson.geometry?.type === 'MultiLineString';
    return geoJson.type === 'FeatureCollection'
        && Boolean(geoJson.features?.length)
        && geoJson.features!.every((feature) => feature.geometry?.type === 'LineString' || feature.geometry?.type === 'MultiLineString');
}
...
export function TripRouteMap({ stopId }: { stopId?: string }) {
    ...
    useEffect(() => {
        ...
        fetch(manifest.route.geoJson, { signal: controller.signal })
            .then((response) => {
                if (!response.ok) throw new Error(`Route request failed with ${response.status}`);
                return response.json();
            })
            .then((value: unknown) => {
                if (!isRouteGeoJson(value)) throw new Error('Route data must contain LineString geometry.');
                setRoute(value);
            })
            .catch((reason: unknown) => {
                if (reason instanceof DOMException && reason.name === 'AbortError') return;
                setError(reason instanceof Error ? reason.message : 'The route could not be loaded.');
            });
        return () => controller.abort();
    }, [manifest.route.geoJson]);
    ...
    useEffect(() => {
        ...
        map.on('load', () => {
            ...
            map.addSource('trip-stops', { type: 'geojson', data: stops });
            map.addLayer({
                id: 'trip-stops',
                type: 'circle',
                source: 'trip-stops',
    ...
    return (
        <figure className="trip-breakout my-10">
            <div className="relative aspect-[16/10] min-h-72 overflow-hidden rounded-md border border-zinc-800 bg-zinc-950">
                ...
                {!error && <div ref={containerRef} role="region" aria-label={mapAlt} className="absolute inset-0" />}
                {!route && !error && (
